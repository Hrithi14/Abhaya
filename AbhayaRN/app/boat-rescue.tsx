...
export default function BoatRescueScreen() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    const priority = calculateBoatRescuePriority({
      waterDepth: "UNKNOWN", waterRising: rising,
      peopleTrapped: trapped, medicalEmergency: medical,
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {location?.available && (
          <View style={s.locationBadge}>
            <Text style={s.locationText}>📍 {location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}</Text>
          </View>
        )}
        ...
        <View style={s.stepper}>
          <TouchableOpacity onPress={() => people > 1 && setPeople(people - 1)} style={s.stepBtn}>
            <Text style={s.stepBtnText}>−</Text>
          </TouchableOpacity>
          <Text style={s.stepValue}>{people}</Text>
          <TouchableOpacity onPress={() => setPeople(people + 1)} style={s.stepBtn}>
            <Text style={s.stepBtnText}>+</Text>
          </TouchableOpacity>
        </View>
        ...
        <TouchableOpacity onPress={handleSubmit} disabled={loading} style={s.submitBtn}>
          {loading
            ...
            : <Text style={s.submitText}>SEND BOAT RESCUE ALERT</Text>
          }
        </TouchableOpacity>
      </ScrollView>
